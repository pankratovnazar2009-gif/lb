...
src/
  ...
  lib/                    utils (cn, formatDate/Time, localeHref, pluralUk), crest (crestFor: назва команди → /upl/<slug>.png),
                          matchStatus (liveMatches/liveNextMatch — див. «Дані завжди актуальні» нижче)
...
`Hero.tsx`: на телефонах (< 640px) — окремий cutout, `Banada` праворуч від вордмарку (`sm:hidden`); від `sm` — `Souza` праворуч, від `md` — обидва (`Banada` зліва + `Souza` праворуч), як на десктопі. Раніше обидва «десктопні» cutout'и були `hidden` нижче `sm`/`md`, тож на мобільних хіро складався з самого лише тексту на порожньому тлі — виправлено додаванням мобільного cutout'а.
...
### Дані завжди актуальні (matchStatus)

`matches.ts` містить фіксовані дати (`kickoff`), тому без додаткової логіки сайт рано чи пізно почав би показувати «СКОРО» для туру, який давно зіграний. `src/lib/matchStatus.ts` це лікує:

- `liveMatches()` бере `results`+`fixtures`, і будь-який матч, чий `kickoff` вже минув, а результату для нього ще не вписано в `matches.ts`, автоматично «дограється» — отримує статус `result` і **детермінований** (стабільний, не випадковий щоразу) заглушковий рахунок з `autoScoreFor(id)`. Це лише запобіжник: щойно в `matches.ts` з'явиться справжній рахунок (і, бажано, склад у `lineups.ts`), він має пріоритет.
- `liveNextMatch()` — перший матч, що ще не настав; на ньому тримається плашка «Найближчий матч» у хедері.
- Використовується в `MatchesTicker`, `MatchesFull`, `SiteHeader` — усі «клієнтські» компоненти, рахують час у браузері відвідувача, тож дата завжди актуальна без пересборки сайту.
- **Через це турнірна таблиця (`table.ts`) — окремий, вручну підтримуваний знімок**, не пов'язаний обчисленням з `matches.ts` (бо там лише розклад «Лівого Берега», а не повний календар усіх 16 клубів). Тури 5–6 вже відіграні (Чорноморець 1:2, потім 2:1 Полісся) — клуб піднявся на 2-ге місце (зона Ліги Європи). Коли реальні результати наступних турів стануть відомі, оновлюйте `matches.ts` (рахунок), `lineups.ts` (склад+голи) і `table.ts`/`playerStats.ts` (турнірне становище й статистика) разом, щоб цифри й далі збігалися.
